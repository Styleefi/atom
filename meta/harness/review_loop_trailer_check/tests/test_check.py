...
@pytest.mark.parametrize("command", ["git -C . commit -q -m x", "FOO=1 git cherry-pick abc", "ls && git revert HEAD"])
def test_authoring_verbs_are_recognised(command: str) -> None:
    assert check._authoring_verb(command) is True
...
def test_non_authoring_commands_are_not(command: str) -> None:
    assert check._authoring_verb(command) is False
