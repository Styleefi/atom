...
가드의 차단·오버라이드 경로는 blocklog에 한 줄을 쓴다. 이 패키지의 테스트는
`main()`을 그대로 실행하므로, 격리가 없으면 pytest 한 번이 오너의 실제 원장
(`~/.local/state/atom/guard-blocklog.jsonl`)에 픽스처 명령 수십 줄을 남긴다 —
#76이 없애려는 "수동 실행을 손으로 걸러내는" 고고학을 첫 커밋부터 재생산하는 셈이다.

`HOME`은 건드리지 않는다: 외부 CLI를 셸아웃하는 테스트의 동작이 함께 바뀐다.
