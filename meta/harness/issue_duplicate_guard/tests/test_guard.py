...
# 적을 때는 무조건 실행되는 경우이고, 조건부면 "실행은 …때" 형식으로 조건을 적는다.
#
...
# 규약을 통째로 빠뜨린 항목은 test_every_corpus_entry_states_execution이 잡는다.
# 그 테스트는 설명이 **사실인지**는 판정하지 못하므로, 새 항목을 넣을 때는 위
# 재현법으로 직접 대조하라.
...
# 발생은 차단 이력 로그(#76)에 자동으로 남고, 이슈 신설·수리는 반복 발생 등
# 실증된 비용을 근거로 한 owner 결정으로만 연다(guard.py 동결 선언 참조).
# 새 항목의 추가 자체는 owner 결정이 필요 없다 — 기록은 무조건이다.
#
# **이 표가 망라적이라는 증명은 없다.** 여기 있는 것은 실측으로 확인된 것들뿐이다.
# 1~3번은 정확 일치 OPERATORS 경로를 지나지만 도달 이유는 다르다 — 1·3번은 인용·
# 이스케이프 때문에 표식 정렬이 깨져 폴백으로 떨어지고, 2번은 표식이 정상인데도
# `;;`가 정확 일치 연산자라서 그 경로가 곧바로 경계를 만든다(case 밖에서는 문법
# 오류라는 bash 문맥 민감성은 이 가드가 보지 않는다). 4번은 토큰화가 주석
# 해석을 끈 대가(리터럴 `#` 보존)이고, 5번은 세그먼테이션 밖 _parse_segment가
# --help를 모르는 것이라 동결 계층의 결함이 아니다.
#
...
_KNOWN_FALSE_BLOCKS = [
    ...
    ("주석 해석이 꺼져 있어(리터럴 `#` 보존) 주석 뒤 텍스트가 명령으로 읽힌다 "
     "— bash: 미실행(`#` 뒤는 주석). 수리하면 낱말 중간 `#`(`feat:#12`, "
     "`${#arr[@]}`)이 잘려 새 오분류가 생긴다(실측) — #74 결정으로 기록만",
     'echo x # ; gh issue create -t "T"', ("T",), "#74 시금석(구성 발견)"),
    ("gh는 --help에서 도움말만 출력하고 생성하지 않는데, _parse_segment가 "
     "--help를 몰라 제목 없는 create로 차단된다 — bash: 실행됨(gh 도움말, "
     "create 없음). 실사용 관측 3회(2026-07-29 2회, 2026-08-02 이 표 작성 중 "
     "1회), 전부 override 1회로 복구 — #74 결정으로 기록만",
     "gh issue create --help", (None,), "#74 결정(실사용 관측)"),
]


# 설명이 bash 실행 상태를 밝히는 세 가지 형식. 무조건 실행이면 "실행됨",
# 무조건 미실행이면 "미실행", 조건부면 "실행은 …때"로 조건을 병기한다
# (`실행은 x가 참일 때`, `실행은 파일 x가 있을 때`처럼 서술어는 자유다).
_EXECUTION_CLAIM_FORMS = ("실행됨", "미실행", "실행은")


def test_every_corpus_entry_states_execution() -> None:
    # 이 테스트가 잡는 것은 **규약을 통째로 빠뜨린 항목**과 조건부 형식의 누락뿐이다.
    # 설명이 사실인지는 검사하지 못한다 — 그건 bash로만 판정할 수 있고 위 헤더의
    # 대조 절차가 담당한다(무한 루프·파일 의존 항목이 있어 스위트에 넣지 않는다).
    # 표식 단어를 create와 무관한 문맥에 써도 이 검사는 통과한다.
    ...
    for table_name, table in tables:
        for entry in table:
            ...
            if "실행은" in description:
                assert "때" in description, (
                    f"{table_name}: 조건부 실행인데 조건이 없다 — {description!r}"
                )
