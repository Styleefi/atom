...
- 오탐 방지가 최우선: 전체 명령을 shlex로 먼저 토큰화해 따옴표 문자열을
  ...
  정확 일치 경로는 폴백에서 인용을 보지 못하므로 오차단이 완전히 사라지지는
  않는다. 남는 조건과 수용 근거는 _protected_flags docstring과 테스트의
  _KNOWN_FALSE_BLOCKS 표에 있다.
...
def _protected_flags(text: str, count: int) -> list[bool] | None:
    ...
    접기는 bash 인용 규칙 **세 개**만 따른다 — 작은따옴표 안은 전부 리터럴(접지
    않는다), 큰따옴표는 작은따옴표를 무효화한다, 백슬래시는 작은따옴표 밖에서만
    이스케이프다. 확장·산술·백틱·heredoc은 모델링하지 않으며, 목적도 구조 해석이
    아니라 **토큰 개수 맞추기**뿐이다(동결된 _mask_arithmetic과 다른 점).

    토큰 수가 그래도 어긋나면 **모른다(None)**를 돌려준다. 호출자는 그때 이
    모듈이 도입한 판정 두 가지를 끄고 정확 일치 연산자만 본다(_split_segments).

    남는 오차단은 세 조건이 동시에 성립할 때다 — (1) 이스케이프된 큰따옴표가
    낱말 경계를 가로지르는 배치(`"a\\" "a\\"`. 균형 잡힌 `"say \\"hi\\""`는
    안전하다), (2) 인용·이스케이프된 연산자 리터럴이 인자 위치, (3) 그 뒤에
    `gh issue create`가 낱말로. owner가 수용한 한계이며 테스트의
    _KNOWN_FALSE_BLOCKS 표가 잠근다. 실사용에서 관측되면 수용을 철회한다.
