...
- 오차단은 **언제나 기록한다** — 구성(리뷰·퍼즈·조사) 발견은 발견한 세션이
  그 자리에서 _KNOWN_FALSE_BLOCKS에, 실사용 발생은 차단 이력 로그(#76)가
  자동으로 남긴다. 게이트되는 것은 수리뿐이다: 이슈 신설·수리·동결 해제는
  반복 발생 등 실증된 비용을 근거로 한 owner 결정 사안이며, 동결을 인용한
  보고 묵살은 허용되지 않는다. 오차단 복구는 언제나 ATOM_DUP_REVIEWED=1
  (관측된 전 사례가 재시도 1회로 복구 — #74 결정 코멘트의 실측).
...
- 오탐 방지가 최우선: 전체 명령을 shlex로 먼저 토큰화해 따옴표 문자열을
  ...
  case 밖에서 문법 오류인 토큰 등). 인용과 무관한 오차단 계열도 있다 — 주석
  해석이 리터럴 `#` 보존을 위해 꺼져 있어(_tokenize) 주석 뒤 텍스트가 명령으로
  읽히므로 `echo x # ; gh issue create -t "T"`(bash 미실행)가 감지·차단되고,
  `gh issue create --help`(gh는 도움말만 출력, 생성 없음)는 세그먼테이션 밖
  _parse_segment가 --help를 모르므로 제목 없는 create로 차단된다(실사용 2회
  관측, 2026-07-29 — #74 결정으로 기록만). 알려진 것은 테스트의
  _KNOWN_FALSE_BLOCKS 표에 잠겨 있으나 망라적이라는 증명은 없다. 조건과 수용
  근거는 _protected_flags docstring 참조.
...
def _protected_flags(text: str, count: int) -> list[bool] | None:
    ...
    **이것이 이 하네스의 오차단 전부라는 뜻은 아니다.** 인용과 무관한 사전 존재
    ...
    수용한 한계다. 실사용 발생은 차단 이력 로그(#76)에 자동으로 남고, 수리
    여부는 그 기록을 근거로 한 owner 결정이다(모듈 docstring의 동결 선언 참조).
