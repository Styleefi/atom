...
집계 해석 노트:
    ...
    - **`override` 줄은 선행 차단의 증거가 아니다.** 마커를 선제적으로 붙였거나
      ...
      기록 순서는 유지된다.
    - **오버라이드는 하니스 사이에서 이중 계상될 수 있다.** 하나의
      `ATOM_COMMIT_OVERRIDE=1 git commit ...`이 commit-guard(PreToolUse)와
      commit-backstop(PostToolUse) 양쪽에서 `override` 줄을 남길 수 있다. #76
      베이스라인 표는 마커별 횟수를 단일 숫자로 세었으므로, 그 표와의 비교는
      `harness` 필드로 나눈 뒤에만 성립한다. **두 줄이 짝을 이룬다고 기대해서는
      안 된다** — backstop의 줄은 저장소 해석·ref 이동·remote 존재를 모두 통과한
      뒤에만 나오므로, 커밋이 실패했거나 remote 없는 스크래치 저장소이거나 payload
      cwd 밖 저장소면 guard 줄만 남는다. 짝이 안 맞는 것은 기록 유실의 증거가
      아니다.
