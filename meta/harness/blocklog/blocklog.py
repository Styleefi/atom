...
주장하는 것:
    - 차단(`event: "block"`), 오버라이드 통과(`event: "override"`), 하니스가
      제 주장을 이행하지 못한 강등(`event: "degraded"`), 그리고 비차단 하니스의
      보고(`event: "report"`)가 발생하면 **최선을 다해** 한 줄을 남긴다.
      `degraded`는 두 갈래를 함께 담는다 — 판정을 내고도 집행하지 못한 경우와,
      평가 자체를 수행하지 못한 경우. 어느 쪽인지는 `reason`이 가른다.
    - 어떤 실패도 호출자에게 전파하지 않는다 (fail-open). 예외를 던지지 않는다.
    - 차단·오버라이드·강등·보고가 없는 경로는 이 함수를 부르지 않으므로 아무것도
      쓰지 않는다. fail-open 경로도 마찬가지지만 `degraded`는 예외다 — 주장이
      적용되지 않았다는 사실 자체가 기록 대상이다(#115).
...
def record_block(
    ...
    Args:
        event: 이벤트 종류 (`"block"`, `"override"`, `"degraded"`, `"report"` — 참여 하니스가
            늘면 확장된다; 위 집계 해석 노트의 전방 호환 규칙 참고).
        harness: 이벤트를 낸 하니스 id (예: `"commit-guard"`).
        reason: 이벤트 분류(차단 사유, 강등 사유). override 이벤트에서만 None.
        command: 훅 페이로드의 Bash 명령 원문. `degraded`와 `report`에서는 None —
            강등은 명령이 아니라 훅 자신의 상태를 서술하므로 원문을 싣지 않는다.
