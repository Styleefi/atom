# meta/harness의 .py와 meta/의 .md가 테스트 이름으로 가리키는 대상이 실재하는지 검증하는 테스트
"""산문 테스트 이름 포인터 해석 테스트 (#101).

meta/harness의 .py와 meta/의 .md(`_PRUNED_DIRS` 아래 제외)가 `test_...` 이름으로
가리키는 테스트가 실재하는지 본다. 테스트를 리네임해도 스위트는 초록이고 그 이름을
가리키던 산문만 썩기 때문이다. 설계와 근거(4라운드 리뷰, 유예, 재진입 판정)는 #101이
보유한다.
...
루트 문서를 제품 문서로 다시 쓰기 때문). 범위 안에서 `_TOKEN_RE`에 걸리는 이름은
코드 식별자·픽스처·예시를 가리지 않고 모두 알려진 이름이어야 한다.
"""
