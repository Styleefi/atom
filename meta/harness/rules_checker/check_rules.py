...
def check_hook_wiring(root: Path) -> list[str]:
    ...
    한계: settings.local.json은 루트 .gitignore가 무시하는 로컬 오버라이드
    자리라 저장소에 올라오지 않는다 — CI 검증 범위 밖(문서화된 한계). 이
    전제를 떠받치는 것은 그 .gitignore 항목 하나뿐이고 체커는 그것을
    검사하지 않는다(#108 — 관례가 강하고, 검사하려면 순수 파일시스템인 이
    체커에 git 의존이 들어온다. 실제 유출이 관측되면 그때 근거가 생긴다).
    읽기·파싱이 불가능한 규칙 파일이 선언한 비표준 deployed-to는 대상을 알
    수 없어 그 파일이 고쳐질 때까지 스윕할 수 없다 — 인벤토리 보류와 같은
    공표된 지연 패턴이며, 그런 규칙 곁에서의 스윕 생존과 규칙 쪽 보고는
    *_does_not_kill_the_sweep 계열 테스트에 있다. 읽기 실패(OSError)·JSON
    파싱 실패·비객체 대상은 규칙 유무와 무관하게 위반으로 소리낸다 —
    규칙별 검사가 frontmatter 오류로 조기 종료하면 아무도 보고하지 않는
    조합이 생기기 때문이며(최종 게이트 리뷰), 건강한 ruled 대상의 이중
    보고는 수용된 패턴이다. 따옴표 감싼 모듈명은 미감지 (_HOOK_MODULE_RE
    주석 참조).
