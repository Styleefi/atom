...
검사 범위는 위 계약 키가 **선언한** 값의 동등성까지다(그 밖의 잡 키는 포지
관용구로서 비교하지 않는다). 선언이 실제로 실행되는지 —
워크플로 트리거, 스텝/잡 조건(``if:``/``rules:``), 환경 변수, 커밋된 도구
설정 — 는 우연히 생길 수 없는 의도적 행위의 영역이라 위협 모델 밖이다
(PR #86 원장에 기록된 트레이드오프). 이 경계 너머를 어휘 검사로 쫓으면
층이 끝없이 이어진다는 것이 리뷰 라운드 3~4의 결론이다.
"""
...
# 리뷰 라운드 1~3에서 층층이 새는 게 증명됐다(플러그인명 오인 → 설치 명령 →
# exit 0 제어 흐름 주입 — 줄의 존재가 줄의 실행을 보장하지 않는다). 목록 전체
