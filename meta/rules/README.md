...
- `enforce: hook`: the rule's enforcement lives in a harness package `meta/harness/<id with - → _>/`, wired into the deployed-to settings JSON. The checker verifies the settings file parses as JSON, that exactly one hook command references the `harness.<id_with_underscores>` module, that this command byte-matches the canonical wrapper template selected by `blocking` (`true` → the 42→2 remapping wrapper, `false` → the never-blocking `|| exit 1` wrapper; #31), and that the harness package exists. A repo-level reverse sweep additionally requires every harness-referencing hook command — ruled or not — to match a canonical template; non-harness commands are out of scope.
