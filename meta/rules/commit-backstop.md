...
`branch-eval-failed`, `head-eval-failed`. `state-unwritable` is a verdict that
was produced and not enforced; the other four mean no verdict was reached at
all, and counting those as suppressed violations inflates exactly the number
this recording exists to observe. `state-unreadable` and `state-corrupt` mean
the state file was there but yielded no usable baseline to compare against. For a held verdict the ledger is the point: it lives outside the git
