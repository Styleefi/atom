...
- Raising the bar (adding a class) needs only a ledger entry through round
  ...
  Authority over the bar is asymmetric: the agent running the loop never
  authors the initial declaration and may raise on the terms above.
...
- A commit in this loop may delete a sentence or its overclaiming clause,
  ...
  and failed to falsify the exact working-tree text and message header to be
...
finding matching an owner-accepted trade-off is closed at triage (not
counted, not filed) unless it carries new evidence, which makes it a normal
finding.
